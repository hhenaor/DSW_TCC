...
if ($news_id > 0) {
	...
	if (!$news_item) {
		$error_msg = "La noticia no existe.";
	}
} else {
	$error_msg = "ID de noticia inválido.";
}
...
<head>
	...
	<title><?php echo htmlspecialchars($news_item['title']); ?> - Foro E</title>
...
<body>
	...
	<main>
		...
		<div class="fcol self-actions">
			<?php if (!empty($news_item['link_url'])): ?>
				<img src="<?php echo htmlspecialchars($news_item['link_url']) ?>" alt="Imagen de la noticia" class="post-thumbnail">
			<?php endif; ?>

			<div>
				<h2><?php echo htmlspecialchars($news_item['title']) ?></h2>
				<small>Publicado por <strong><?php echo htmlspecialchars($news_item['username']) ?></strong></small>
			</div>
		</div>
		...
		<div id="comments">
			<h3>Comentarios</h3>
			<?php
			$comments = get_news_comments_list($news_id);
			if (empty($comments)) {
			...
			<?php if (isset($_SESSION['user_id'])): ?>
				<div class="fcol">
					...
					<form action="php/comment_manager.php" method="post" class="fcol">
						...
						<input type="hidden" name="content_id" value="<?php echo $news_id; ?>">
						...
						<div class="frow">
							...
							<a href="noticia.php?id=<?php echo $news_id; ?>" class="btn red sml" style="margin-left: 1em; text-decoration: none; text-align: center;">Cancelar</a>
						</div>
					</form>
				</div>
			<?php else: ?>
		...
		<?php else: ?>
			<div class="frow">
				<a href="noticia.php?id=<?php echo $news_id; ?>&comment=new">
